...
namespace PDFExporterLib
{
    public class PDFBuilder
    {
        ...
        public class HtmlToPdfBuilder
        {
            ...
            private const string REGEX_GET_STYLES = @"(?<selector>[A-Za-z]+[\s]*)[\s]*{[\s]*((?<properties>[A-Za-z0-9-_]+)[\s]*:[\s]*(?<style>[A-Za-z0-9#, ]+);[\s]*)*[\s]*}";
            #endregion
            ...
            public void ImportStylesheet(string path)
            {
                ...
                {
                    string css = match.Value.Replace("\r\n", "");
                    string[] matchValues = css.Split(new Char[] { '{', '}' });
